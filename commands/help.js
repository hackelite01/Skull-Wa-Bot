...
const execute = async (client,msg,args) => {
    ...
    if(!args.length){
        let adminHelp = '⚜️ *Administration*\n\n';
        let infoHelp = '⚜️ *Info*\n\n';
        let pluginHelp = '⚜️ *Plugins*\n\n';
        commands.forEach((command) => {
            if(!command.isDependent){
                if(command.commandType === 'admin')
                    adminHelp += `✳️ *${command.name} (${command.command})*  - ${command.description}\n`;
                if(command.commandType === 'info')
                    infoHelp += `✳️ *${command.name} (${command.command})*  - ${command.description}\n`;
                if(command.commandType === 'plugin')
                    pluginHelp += `✳️ *${command.name} (${command.command})*  - ${command.description}\n`;
            }
